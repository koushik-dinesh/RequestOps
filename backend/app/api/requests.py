...
WORKLOAD_ACTIVE_STATUSES = ["ASSIGNED", "IN_DEVELOPMENT", "DEVELOPMENT_COMPLETE", "IN_TESTING", "TEST_FAILED", "UAT_PENDING", "UAT_REJECTED"]


def is_admin_or_it(user: dict) -> bool:
    return user["role_code"] in ["SYSTEM_ADMIN", "IT_HEAD", "PROJECT_MANAGER"]
...
def annual_roi_summary(roi: dict) -> str:
    if roi.get("roiType") == "TIME_SAVINGS":
        annual = float(roi.get("roiHoursSavedPerEmployeePerMonth") or 0) * int(roi.get("roiEmployeesBenefited") or 0) * 12
        return f"{annual:g} Hours/Year"
    if roi.get("roiType") == "COST_SAVINGS":
        annual = float(roi.get("roiMonthlyCostSavingsInr") or 0) * 12
...
def create_request(payload: RequestCreatePayload, request_context: FastAPIRequest, background_tasks: BackgroundTasks, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if payload.roiType:
        audit(db, actor_user_id=user["id"], action="ROI_CREATED", entity_type="REQUEST", entity_id=request_id, new_value={
            "roiType": payload.roiType,
...
def update_roi(request_id: int, payload: RoiPayload, request_context: FastAPIRequest, background_tasks: BackgroundTasks, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    notify(db, recipient_user_id=request_row["requester_user_id"], request_id=request_id, type="ROI_UPDATED", title="ROI information updated", message=f"ROI was updated for {request_row['request_number']}.", background_tasks=background_tasks)
...
def it_review_approve(request_id: int, payload: ItReviewApprovePayload, request_context: FastAPIRequest, background_tasks: BackgroundTasks, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    updated = transition_request(db, request_id=request_row["id"], to_status="ASSIGNMENT_PENDING", actor_user_id=user["id"], comment=payload.comment or "Internal review approved. Request moved to waiting for assignment.", request_context=request_context, patch={"it_head_user_id": user["id"], "current_assignee_user_id": user["id"], "feasibility_notes": payload.feasibilityNotes, "complexity": payload.complexity, "estimated_effort": payload.estimatedEffort, "priority_confirmation": payload.priorityConfirmation, "priority": payload.priorityConfirmation})
    notify(db, recipient_user_id=user["id"], request_id=request_row["id"], type="REQUEST_ASSIGNMENT_PENDING", title="Waiting for assignment", message=f"{request_row['request_number']} is approved and ready for team assignment.", background_tasks=background_tasks)
    db.commit()
    return ok(updated)
...
def assign(request_id: int, payload: AssignPayload, request_context: FastAPIRequest, background_tasks: BackgroundTasks, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    request_row = assert_request_access(db, user, request_id, ["IT_HEAD", "PROJECT_MANAGER"])
    if request_row["status"] not in ["ASSIGNMENT_PENDING", "ASSIGNED"]:
        raise ApiError(409, "Resources can only be assigned while assignment is pending or already completed.")
    ...
    assignment_comment = f"Assignment updated. {' '.join(changes)}" if previous and changes else ("Assignment updated." if previous else f"Team assigned. Assigned team member {developer['full_name']}. Reviewer {qa['full_name']}. Status changed to Team Assigned.")
    ...
    execute(db, "UPDATE requests SET status = 'ASSIGNED', current_assignee_user_id = :developerUserId, it_head_user_id = :itHeadUserId WHERE id = :requestId", {"requestId": request_row["id"], "developerUserId": payload.developerUserId, "itHeadUserId": user["id"]})
    execute(db, """
        INSERT INTO request_status_history (request_id, from_status, to_status, changed_by_user_id, comment)
        VALUES (:requestId, :fromStatus, 'ASSIGNED', :actorUserId, :comment)
    """, {"requestId": request_row["id"], "fromStatus": request_row["status"], "actorUserId": user["id"], "comment": payload.notes or assignment_comment})
...
def development_complete(request_id: int, payload: OptionalCommentPayload, request_context: FastAPIRequest, background_tasks: BackgroundTasks, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    updated = transition_request(db, request_id=request_row["id"], to_status="DEVELOPMENT_COMPLETE", actor_user_id=user["id"], comment=payload.comment or "Work complete.", request_context=request_context, patch={"progress_percentage": 100, "current_assignee_user_id": assignment.get("qa_user_id") if assignment else None})
    transition_request(db, request_id=request_row["id"], to_status="IN_TESTING", actor_user_id=user["id"], comment="Routed to review and validation.", request_context=request_context, patch={"current_assignee_user_id": assignment.get("qa_user_id") if assignment else None})
...
def testing_result(request_id: int, payload: TestResultPayload, request_context: FastAPIRequest, background_tasks: BackgroundTasks, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if payload.result == "PASS":
        ...
        updated = transition_request(db, request_id=request_row["id"], to_status="UAT_PENDING", actor_user_id=user["id"], comment=payload.testSummary, request_context=request_context, patch={"current_assignee_user_id": uat_user_id})
        ...
        return ok(updated)
    assignment = get_active_assignment(db, request_row["id"])
    failed = transition_request(db, request_id=request_row["id"], to_status="TEST_FAILED", actor_user_id=user["id"], comment=payload.testSummary, request_context=request_context, patch={"current_assignee_user_id": assignment.get("developer_user_id") if assignment else None})
    transition_request(db, request_id=request_row["id"], to_status="IN_DEVELOPMENT", actor_user_id=user["id"], comment="Returned for changes after review.", request_context=request_context, patch={"current_assignee_user_id": assignment.get("developer_user_id") if assignment else None})
...
def testing_clarification(request_id: int, payload: ClarificationPayload, request_context: FastAPIRequest, background_tasks: BackgroundTasks, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    request_row = assert_request_access(db, user, request_id, ["QA"])
    if request_row["status"] != "IN_TESTING":
    ...
    updated = create_clarification_request(db, request_row=request_row, actor_user_id=user["id"], reason_category=payload.reasonCategory, note=payload.note, return_status="IN_TESTING", return_assignee_user_id=user["id"], request_context=request_context, background_tasks=background_tasks)
    db.commit()
    return ok(updated)
...
def uat_approve(request_id: int, payload: UatApprovePayload, request_context: FastAPIRequest, background_tasks: BackgroundTasks, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    updated = transition_request(db, request_id=request_row["id"], to_status="CLOSED", actor_user_id=user["id"], comment=payload.comments or "Final approval completed and request completed.", request_context=request_context, patch={"current_assignee_user_id": None})
...
def uat_reject(request_id: int, payload: UatRejectPayload, request_context: FastAPIRequest, background_tasks: BackgroundTasks, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    rejected = transition_request(db, request_id=request_row["id"], to_status="UAT_REJECTED", actor_user_id=user["id"], comment=payload.comments, request_context=request_context, patch={"current_assignee_user_id": assignment.get("developer_user_id") if assignment else None})
    transition_request(db, request_id=request_row["id"], to_status="IN_DEVELOPMENT", actor_user_id=user["id"], comment="Returned for changes after final approval review.", request_context=request_context, patch={"current_assignee_user_id": assignment.get("developer_user_id") if assignment else None})
