...
def dashboard_me(user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if role == "SYSTEM_ADMIN":
        cards.extend([
            ...
            {"label": "Planning", "value": scalar(db, "SELECT COUNT(*) AS count FROM requests WHERE status IN ('PM_ASSIGNED','SCOPE_REVIEW','USER_STORY_REVIEW','DEVELOPER_ASSIGNED','SPRINT_PLANNING')", params)},
        ...
        cards.extend([
            ...
            {"label": "User Story Review", "value": scalar(db, "SELECT COUNT(*) AS count FROM requests WHERE requester_department_id = :departmentId AND status = 'USER_STORY_REVIEW'", params)},
        ...
        cards.extend([
            ...
            {"label": "Average Completion %", "value": scalar(db, "SELECT COALESCE(ROUND(AVG(progress_percentage)), 0) AS count FROM requests WHERE status IN ('DEVELOPER_ASSIGNED','SPRINT_PLANNING','ASSIGNED','IN_DEVELOPMENT','DEVELOPMENT_COMPLETE')", params)},
        ...
        cards.extend([
            ...
            {"label": "Scope / Story Planning", "value": scalar(db, "SELECT COUNT(*) AS count FROM requests WHERE project_manager_user_id = :userId AND status IN ('PM_ASSIGNED','SCOPE_REVIEW','USER_STORY_REVIEW')", params)},
            {"label": "Ready For Developer", "value": scalar(db, "SELECT COUNT(*) AS count FROM requests WHERE project_manager_user_id = :userId AND status = 'USER_STORY_REVIEW'", params)},
            {"label": "Sprint Planning", "value": scalar(db, "SELECT COUNT(*) AS count FROM requests WHERE project_manager_user_id = :userId AND status IN ('DEVELOPER_ASSIGNED','SPRINT_PLANNING')", params)},
        ...
        cards.extend([
            ...
            {"label": "Ready To Start", "value": scalar(db, "SELECT COUNT(*) AS count FROM requests r JOIN assignments a ON a.request_id = r.id AND a.is_active = TRUE WHERE a.developer_user_id = :userId AND r.status IN ('ASSIGNED','DEVELOPER_ASSIGNED','SPRINT_PLANNING')", params)},
        ...
        cards.extend([
            ...
            {"label": "In Progress", "value": scalar(db, "SELECT COUNT(*) AS count FROM requests WHERE requester_user_id = :userId AND status IN ('ASSIGNED','DEVELOPER_ASSIGNED','SPRINT_PLANNING','IN_DEVELOPMENT','DEVELOPMENT_COMPLETE','QA_PENDING','QA_PASSED','IN_TESTING','UAT_PENDING','DEPLOYMENT_PENDING','DEPLOYED')", params)},
